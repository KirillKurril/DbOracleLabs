...
BEGIN
  ...
  ELSIF v_query_type IN ('INSERT', 'UPDATE', 'DELETE') THEN
    ...
    p_message := 'DML operation ' || v_query_type || ' complete.';
    p_cursor  := NULL;
  ...
  ELSIF v_query_type = 'DDL' THEN
    ...
    IF v_ddl_command = 'CREATE TABLE' THEN
      ...
      EXECUTE IMMEDIATE v_query;
      p_message := 'Table ' || v_table || ' created successfully.';
      p_rows    := 0;
      p_cursor  := NULL;
    ...
    ELSIF v_ddl_command = 'DROP TABLE' THEN
      ...
      EXECUTE IMMEDIATE v_query;
      p_message := 'Table ' || v_table || ' is deleted successfully.';
      p_rows    := 0;
      p_cursor  := NULL;
    ELSE
      RAISE_APPLICATION_ERROR(-20001, 'DDL command is not supported: ' || v_ddl_command);
    END IF;

  ELSE
    RAISE_APPLICATION_ERROR(-20001, 'Request type does not supported: ' || v_query_type);
  END IF;
